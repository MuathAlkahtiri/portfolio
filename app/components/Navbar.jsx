  ...
  return (
    <>
      ...
      <nav className={`w-full fixed px-5 lg:px-8 xl:px-[8%]
        py-1 flex items-center justify-between z-50 
        ${isScroll
          ? "bg-white/50 backdrop-blur-lg shadow-sm dark:bg-darkTheme/50 dark:shadow-white/20"
          : ""}`}>

        <a href="#top">
          <Image src={theme === 'dark' ? assets.logo_dark : assets.logo} alt="" className="w-28 cursor-pointer mr-14" />
        </a>
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}>
            <Image src={theme === 'dark' ? assets.sun_icon : assets.moon_icon} alt="" className="w-6 cursor-pointer" />
          </button>

          <a href="#contact" className="hidden lg:flex items-center gap-3 px-10 py-2.5 border
            border-gray-500 rounded-full ml-4 font-Ovo text-black dark:border-white/50 dark:text-white">
            Contact
